...
- `INPUTPOS|x|y` — put the pointer at an absolute screen position via `IECLASS_POINTERPOS` (v1.22+); `INPUTMOVE` deltas are subject to Input Prefs mouse acceleration, `INPUTPOS` is exact
